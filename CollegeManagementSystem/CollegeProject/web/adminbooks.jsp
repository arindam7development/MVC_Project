...
<s:query var="rs" sql="select * from sub_detail"/>
<table width="300" align="center" bgcolor="lightgray" border="1" align="left">
    ...
    <tr bgcolor="white">
        ...
        <td>${data.branch}</td>
        <td>
           <a href="DeleteSubId.do?bid=${data.sub_id}">Delete</a></td>
     </tr>
    </c:if>
    <c:if test="${s.count%2 != 0}">
    <tr>
        ...
        <td>${data.branch}</td>
        <td>
            <a href="DeleteSubId.do?bid=${data.sub_id}">Delete</a>
        </td>
     </tr>
    </c:if>
    </c:forEach>
        <h:form action="addsubid">
    <tr>
        <td><h:text property="sid"/></td>
        <td><h:text property="sname"/></td>
